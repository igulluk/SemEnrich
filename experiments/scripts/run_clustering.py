"""Script to run clustering on medical findings."""
    ...
    for i, sample in enumerate(all_data):
        ...
        try:
            ...
            if findings.endswith("."):
                findings_list = findings.split(".")[:-1]
            else:
                findings_list = findings.split(".")
            
            findings_list = [s.lower().strip() for s in findings_list if s.strip()]
            embeddings = data['embeddings_findings']
            
            if len(findings_list) != len(embeddings):
                continue
            
            for text, embedding in zip(findings_list, embeddings):
                thought_dict[text] = embedding
            
            training_data.append({
                'Findings': sample['Findings'],
                'embeddings_findings': embeddings
            })
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run clustering on medical findings")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
